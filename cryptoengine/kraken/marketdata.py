...
def get_value(pair):
    ...
    if len(pair) <= 4:
        raise MarketDataException(
            "Invalid pair length. Pair must be longer than 4 characters. Try to add ZEUR, EUR, ZUSD, USD.")
    ...
    try:
        ...
        raise MarketDataException(e) from e
...
def get_ticker(pair):
    ...
    try:
        ...
        logging.error('%s %s', pair, str(e).replace('\n', ' '))
        raise MarketDataException(str(e).replace('\n', ' ')) from e
